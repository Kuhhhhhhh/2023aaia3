//(寫到一半)
//week 07-2c.cpp 1275. Find Winner on a Tic Tac Toe Game
class Solution {
public:
    ...
    bool myTestWin(int board[3][3],int now){
        ...
        //對角線
        if(board[0][0]==now && board[1][1]==now && board[2][2]==now) return true;//左上角
        if(board[0][2]==now && board[1][1]==now && board[2][0]==now) return true;//右上角
